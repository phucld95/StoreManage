            ...
            public void actionPerformed(ActionEvent e){
            	...
            	if(name.length() != 0){
					...
					try {
						...
						//System.out.print(rs.getRow());
						if(rs.getRow() == 0){
							JOptionPane.showMessageDialog(null, "Không có nhà cung cấp có tên cần sửa!");						
						}	
						rs.beforeFirst();
						if(rs.getRow() > 1){
							rs.beforeFirst();
							TableDatabase stt = new TableDatabase(rs);
							JOptionPane.showMessageDialog(null, "Mời nhập ID nhà cung cấp để sửa thông tin!");
						}
						else{
							if(chose == cb11){
								sql = "UPDATE ncc SET Ten_NCC = '" + newIn + "' WHERE Ten_NCC = '" + name +"';";
...
//								else{
									if(chose == cb12){
										sql = "UPDATE ncc SET TTLH = '" + newIn + "' WHERE Ten_NCC = '" + name +"';";
										st.executeUpdate (sql);
...
//           				}
					...
					try {
						...
						//System.out.print(rs.getRow());
						if(rs.getRow() == 0){
							JOptionPane.showMessageDialog(null, "Không có nhà cung cấp có mã số giống mã số nhập!");						
						}	
						else{
							if(chose == cb11){
